...
#  col_order <- column_order(p)  
  ...
  aGrid <- grid.arrange(fviz_grob, heatmap_grob, ncol = 2, widths = c(240,240),heights = c(480))
  ...
  selectedSirPheatmapGrob <- add_grob(selectedsirPheatmap)
  selectedPrimaryPheatmapGrob <- add_grob(selectedPrimaryPheatmap)
  
  aGrid <- grid.arrange(selectedPrimaryPheatmapGrob,selectedSirPheatmapGrob, ncol = 2, widths = c(260,220),heights = c(480))
  #aGrid <- grid.arrange(selectedSirPheatmapGrob,selectedPrimaryPheatmapGrob, ncol = 2, widths = c(220,260),heights = c(480))
  dumpGridObject(name="HeatmapBetalactamase",aGrid)
...
printATUIsolates <- function()
{
  atu <- readATUAntibioticsModel()
  nonATUisolates <- rownames(atu)[!apply(atu,1,any)]
  print(paste("c(",paste(nonATUisolates,collapse = ", "),")"))
