...
includedMillimetertable <-function()
{
  readPhenotypeTable() %>% filter(is.na(Exkludera)) %>% select(all_of(allAntibioticsWithZonesInputColumns))
}

modelMilimeterTable <- function()
{
  headingTable <- readHeadingToAntibioticTable()
  includedMillimetertable() %>% select(!!!setNames(headingTable$Heading, headingTable$Antibiotic))
}
...
head(includedMillimetertable())
head(modelMilimeterTable())
